...
import { getGameAdministrators } from 'apiSdk/game-administrators';
import { GameAdministratorInterface } from 'interfaces/game-administrator';
import { getCustomers } from 'apiSdk/customers';
import { CustomerInterface } from 'interfaces/customer';
import { getCustomerSupportRepresentatives } from 'apiSdk/customer-support-representatives';
import { CustomerSupportRepresentativeInterface } from 'interfaces/customer-support-representative';

import { CustomerSupportRepresentativeListPage as ShowCaseList } from 'pages/customer-support-representatives';

function DashboardPage() {
  const { session } = useSession();

  const gameAdministratorStat = useSWR<PaginatedInterface<GameAdministratorInterface>>(
    () => `/game-administrators`,
    () => getGameAdministrators({ limit: 1, offset: 0 }),
  );
  ...
  const customerSupportRepresentativeStat = useSWR<PaginatedInterface<CustomerSupportRepresentativeInterface>>(
    () => `/customer-support-representatives`,
    () => getCustomerSupportRepresentatives({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Game Administrators',
      result: gameAdministratorStat?.data?.totalCount,
      isLoading: gameAdministratorStat.isLoading,
    ...
    },
    {
      ...
      result: customerSupportRepresentativeStat?.data?.totalCount,
      isLoading: customerSupportRepresentativeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Customer Support Representatives
                </Text>
                <NextLink href={`customer-support-representatives`} passHref legacyBehavior>
                  <Button
                  ...
                  >
                    See all Customer Support Representatives
                  </Button>
                </NextLink>
              </Flex>
