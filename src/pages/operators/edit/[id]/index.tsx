...
import { getOperatorById, updateOperatorById } from 'apiSdk/operators';
import { operatorValidationSchema } from 'validationSchema/operators';
...
function OperatorEditPage() {
  ...
  const handleSubmit = async (values: OperatorInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      await imageUploaderRef.current.handleUpload();
      const updated = await updateOperatorById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<OperatorInterface>({
    initialValues: data,
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper wrapperProps={{ mb: 3 }}>
          <ImagePicker ref={imageUploaderRef} onChange={handleImageChange} entity="operator" entityId={id} />
        </FormWrapper>
