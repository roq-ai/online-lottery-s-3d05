...
import { getLotteryGameById } from 'apiSdk/lottery-games';
import { LotteryGameInterface } from 'interfaces/lottery-game';

function LotteryGameViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('lottery_game', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/lottery-games/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Jackpot" text={data?.jackpot} />

                <FormListItem
                  label="Draw Date"
                  text={data?.draw_date ? format(parseISO(data?.draw_date as unknown as string), 'dd-MM-yyyy') : ''}
                />

                <FormListItem
                  label="Draw Time"
                  text={data?.draw_time ? format(parseISO(data?.draw_time as unknown as string), 'dd-MM-yyyy') : ''}
                />
                ...
                {hasAccess('operator', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Operator"
                    text={
                      <Link as={NextLink} href={`/operators/view/${data?.operator?.id}`}>
                        {data?.operator?.name}
                      </Link>
                    }
                  />
