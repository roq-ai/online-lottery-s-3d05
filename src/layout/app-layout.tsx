...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['game-administrator', 'customer-support-representative'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<OperatorInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/operators` : null),
    () => getOperators({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Operators',
      path: isTenantUser ? `/operators/view/${data?.[0]?.id}` : '/operators',
      entity: 'operator',
    ...
    },
    {
      ...
      path: '/customer-support-representatives',
