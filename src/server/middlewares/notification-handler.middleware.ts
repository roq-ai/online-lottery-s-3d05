...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'Support Ticket Submission.create': {
    roles: ['game-administrator', 'customer-support-representative'],
    key: 'support_ticket_submission',
    tenantPath: [],
    userPath: [],
  },
  'Support Ticket Resolution.update': {
    roles: ['game-administrator'],
    key: 'support_ticket_resolution',
    tenantPath: [],
    userPath: [],
  },
  'New Purchase.create': { roles: ['game-administrator'], key: 'new_purchase', tenantPath: [], userPath: [] },
};

const ownerRoles: string[] = ['game-administrator'];
const customerRoles: string[] = ['customer'];
const tenantRoles: string[] = ['game-administrator', 'customer-support-representative'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.operator.findFirst({
